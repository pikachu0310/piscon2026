...
func getIsuList(c echo.Context) error {
	...
	err = db.Select(&rows, `
		SELECT
			i.id, i.jia_isu_uuid, i.name, i.character,
		...
		FROM isu AS i
		LEFT JOIN isu_condition AS c ON c.id = (
			SELECT c2.id
	...
	for _, row := range rows {
		...
		if row.ConditionID.Valid {
			...
			formattedCondition = &GetIsuConditionResponse{
				JIAIsuUUID:     row.JIAIsuUUID,
				IsuName:        row.Name,
				Timestamp:      row.ConditionTimestamp.Time.Unix(),
				IsSitting:      row.ConditionIsSitting.Bool,
				Condition:      row.Condition.String,
				ConditionLevel: conditionLevel,
				Message:        row.ConditionMessage.String,
			}
		}
...
func getTrend(c echo.Context) error {
	...
	for i := range res {
		...
		sort.Slice(res[i].Warning, func(a, b int) bool {
			return res[i].Warning[a].Timestamp > res[i].Warning[b].Timestamp
		})
		sort.Slice(res[i].Critical, func(a, b int) bool {
			return res[i].Critical[a].Timestamp > res[i].Critical[b].Timestamp
		})
	}
