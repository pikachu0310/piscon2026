...
| E17 | 16:39 | pending | condition writeを2msだけqueueで束ね、最大256req/2,000行を1 transactionで保存する | E16でhistory INSERT、latest upsert、START/COMMITが各6.8万回。write SQLだけで160秒、fgprof condition累積679秒 | pending | pending | transaction/query数を大幅削減し、100ms timeout・1秒整合・validatorを守って86,582超なら採用。queue/500/減点ならrollback |
