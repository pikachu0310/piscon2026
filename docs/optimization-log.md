...
| E30 | 18:04 | pending | condition POSTだけを互換JSON decoderへ置換 | E29/E28で標準JSON decode 8.3〜8.7秒、App CPUの約22%。DB idle約90%、read APIは数ms | pending | pending | standard互換設定、同一struct/tag/validationを維持。test/vetとvalidator必須。score/CPUが改善しなければ標準decoderへ戻す |
