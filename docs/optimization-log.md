...
| E23 | 17:16 | `bf25365` | condition POSTをbounded queue受付時に202応答し、DB commitを4 writerへ非同期化 | E22はApp/DBとも約50% idleなのにhandler待ち累積2.99万秒、timeout 3,034。反映猶予は1秒 | pending | pending | validatorとinitialize drainを必須確認。condition timeoutと全API tailを減らし、98,183/99,897を更新すれば採用 |
