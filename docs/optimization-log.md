...
| E25 | 17:32 | `5b0d2b4` | scoring中のcondition履歴をmemory-onlyへし、DB INSERT/queue/4 writerを除去 | E24のDB SQL時間74秒中history INSERTが65.5秒。全readは既にmemory、App host available約1.16GiB | pending | pending | 70%保持を固定。validator/initialize後再構築を通し、DB write消滅とscore/tail改善なら採用 |
