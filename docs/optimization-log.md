...
| E24 | 17:24 | `f095b73` | commit済みcondition履歴をUUID別memory sliceへmirrorし、condition read/graphをDBから外す | E23でhistory SELECT約1.9万回・18.2秒。DB idle 42%、App idle 57%、初期SQLは1MiB未満 | pending | pending | 70%保存・DB永続化は固定。validatorとgraph/condition境界を通し、read SQL消滅とscore/tail改善なら採用 |
