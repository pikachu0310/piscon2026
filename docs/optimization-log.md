...
| E14 | 16:20 | pending | MariaDBのAUTO_INCREMENT割当をinterleaved modeへ変更し、conditionの並列multi-row INSERTを直列化しない | E13でcondition INSERTは6.63万回・64.3万行・133.1秒。`innodb_autoinc_lock_mode=1`、row lock wait 333回 | pending | pending | INSERT SQL時間・row lock wait・timeoutが減り、validかつ83,411を上回れば採用。悪化ならmode 1へrollback |
