...
| E22 | 17:08 | `59b207b` | 最新conditionのbenchmark中の更新・参照をDB tableから同期memory mapへ移す | E21でlatest upsert 1.86万回・12.4秒、historyとの同一transactionでtailを増幅。初期値はinitializeで確定可能 | pending | pending | 70%保存を固定。latest upsert消滅、timeout低下、validを確認し、99,897超なら採用 |
