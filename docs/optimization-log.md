...
| E12 | 16:07 | pending | owner+UUID単位でimmutable icon BLOBを登録時からmemory cacheする | E11 pprofでicon 12.9%。DB response 1.52GiB/56秒の大半が反復BLOB。icon更新routeなし | pending | pending | validかつ76,960超、icon query/DB network/CPUが減れば採用。権限・画像不整合ならrollback |
