...
| E34 | 18:45 | pending | condition routeだけclient abort後もupstream処理を完了 | 公式posterは送信前に期待conditionへ追加し、100ms timeout結果を無視。access logに多数の499があり、既定Nginxはupstreamを切断 | pending | pending | 受信済みbodyのlate処理だけ。invalid body/status/全APIは固定。retained timestamps、Graph/Read score、validator、resourceを確認 |
