...
| E15 | 16:27 | pending | initialize時に全ISU UUIDをmemoryへ読み、condition POSTの存在確認SQLを除く | E14で同一`SELECT COUNT(isu)`が6.33万回・17.2秒。ISU追加APIは1つ、削除APIなし | pending | pending | SELECTをほぼ全廃し、validかつ85,339超またはtimeout/resource明確改善で採用。404差異ならrollback |
