...
| E20 | 16:56 | pending | batch writerを1本から4本へ増やし、60%保存のqueue待ちをDBの並列実行へ移す | E19はDB/App idle 56%/54%なのにcondition最大5.2秒、他API p95約1秒。history SQL concurrencyは0.34 | pending | pending | handler/socket滞留と全API tail、timeoutを減らし、保存行と94,366を増やせば採用。DB飽和/lock悪化なら2本へ縮小 |
