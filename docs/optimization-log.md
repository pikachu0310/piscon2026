...
| E35 | 18:52 | rollback済み | immutable JS/CSSを事前gzipし、Nginx `gzip_static`で配信 | E30 vendor JS 743KiBを1,522回、p95 690ms。gzip-6で203KiB（72%減）、Nginx host idle 65%、gzip_static moduleあり | 公式benchmark（本文参照） | **133,148 / PASSED**（減点44、timeout 442） | 不採用。serverの転送量は72%減ったが、負荷生成量とscoreが低下。E52で誤って再試行し、同じ悪化を2回再現 |
...
### E52 repeat result / rollback

- 同一app/configの公式benchmark `44e05631-d04e-4728-ab85-4523070fba33` は128,492点、PASSED、減点1、timeout 331。計測runは`20260720T041547.603332Z-s1-44e088`で、全hostが`ANALYZED`、全`errors.txt`は空だった。
