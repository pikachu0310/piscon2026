...
| E11 | 16:01 | pending | 署名済みsession cookieの検証結果をinitialize世代内でcacheする | E10でuser存在確認9.26万query。pprofはcookie gob decodeを含むsession処理20.6%、icon処理21.7% | pending | pending | validかつ74,909超、SELECT user/session CPUが激減すれば採用。signout/initialize不整合ならrollback |
