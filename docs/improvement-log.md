...
- icon API平均: 44ms → 6ms
- `/api/user/me`平均: 22ms → 5ms
- signout平均: 21ms → 5ms
- condition POST: 123,146回 → 186,955回
- timeout過多のERRORからPASSEDへ回復

画像cacheだけでは残っていた共通のDB queryを消したことで、軽いAPIは数msで返るようになりました。利用者増加によりDBへ届くconditionは約1.5倍になっています。

## 16. 最新conditionをcacheし、一覧のN+1を解消する

### 見た計測結果

- `/api/isu`: 2,745回、平均271ms、p95 1秒
- user確認をcacheしても、このAPIにはISUごとの最新condition SELECTが残る
- loop 12の相関subquery JOINは平均232msへ悪化し、不採用にした
- conditionの書き込み時には、trend cache用に最新値をすでに判定している

SQLで無理に1本へまとめるのではなく、書き込み時に最新conditionを覚えておけば、一覧requestでcondition tableを読む必要がありません。
...
- run ID: `loop17-relaxed-commit-retest`
- commit: `97c75b8`
- score: **16,632（Portal表示はERROR）**
- 前回比: **+52（+0.3%）**
- 計測error: 4（RUNNINGから120秒collectorを開始したため、60秒の負荷終了時に固定長collectorが残った）

主な変化:

- COMMIT平均: 5.16ms → 0.31ms
- COMMIT合計: 101秒 → 5.11秒
- `/api/isu`平均: 215ms → 190ms
- condition POST回数: 180,672回 → 149,220回
- Portal判定: PASSED → timeout過多のERROR

COMMIT単体は再び大幅に短縮しました。しかしscore差は誤差に近く、処理できたcondition数は減り、Portal判定も悪化しています。耐久性を落としてまで採用する根拠がないため、設定を1へ戻します。

計測開始をRUNNINGへ合わせる場合、collectorは75秒程度で十分だと分かりました。120秒や180秒にすると、ベンチ終了時に固定長pprofを回収できません。

## 18. Nginx静的配信を外し、負荷の増幅を抑える

### 見た計測結果

- Nginx静的配信そのものはassetを1ms未満にした
- しかし導入直後はcondition POSTが約12%増え、DB接続待ちとtimeoutが急増
- scoreは15,095 → 10,804へ下がった
- cache改善後も16,580前後で、静的配信前の18,886を下回る

このベンチでは画面表示が速くなると利用者が早く増え、まだ詰まっているcondition書き込みへ負荷が集中します。局所的に正しい改善でも、システム全体の許容量を超えるとscoreを下げます。
