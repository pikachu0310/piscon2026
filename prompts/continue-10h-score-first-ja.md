...
hard freezeは探索を止めてfinalへ移る時刻である。運営指定がなければ競技終了/portal停止/追試開始の
うち最も早い時刻の60分前（最低でも45分前）とする。hard freezeからscore champion復元、全台reboot、
final確認、公式benchmarkへ移る。開始時点からhard freezeまで10時間未満なら、10時間契約をそのまま開始
...
deadline確認直後、serverへ触る前にtoolkitへ絶対時刻を設定する。

```bash
cd /home/pikachu0310/github/isucon-agent-kit
bin/isuctl guard configure \
  --freeze-at '<RFC3339>' \
  --benchmark-close-at '<RFC3339>' \
  --no-access-at '<RFC3339>' \
  --source '<manual/当日連絡の根拠>'
bin/isuctl guard status
```

deadlineのない明示的な練習環境だけ`guard practice`を使う。guard未設定または拒否を、直接SSH、
Chrome、別scriptで迂回しない。公式benchmarkのbrowser click/POST直前には毎回
`bin/isuctl guard check benchmark`を実行し、許可された同じ作業単位でPOSTを1回だけ行う。
...
6. `bin/isuctl guard check benchmark`を通してからportal preflight/POSTをちょうど1回行う。guard拒否を
   Chromeで迂回しない。応答が曖昧でも自動retryせず、accepted benchmark IDを回収する。
...
## hard freezeからのfinal作業（最低45分を確保）
